...
class Node:
    ...
    def roll_capacity(self, dice_count=1):
        # Only pre-queues skip dice. Towers DO roll dice (occupancy cap is separate).
        if self.is_pretower:
            self.dice_rolls = []
            self.capacity = 0
            return
    ...
    def _bypass_pretower_if_tower_has_space(self, node_map, next_stop_name):
        ...
        bypass the pre-queue and push directly into the Tower. Returns tower name or False.
        """
    ...
    def move_ready_aircraft(self, node_map):
        ...
        for aircraft in list(self.queue):
            ...
            if aircraft.route:
                ...
                # Arrived at final destination gate
                self.queue.remove(aircraft)
                aircraft.beads = 0
...
2) 💎 Distribute Beads (round robin, FIFO order)  
3) ✈️ Move  
...
# Node status overview (force consistent string types)
node_status = []
