...
if st.button("Run Turn"):
    ...
    # Step 1: Gate rolls and injects aircraft
    gate = st.session_state.nodes[0]
    release_count = gate.roll_capacity()
    for _ in range(release_count):
        gate.queue.append("Aircraft")
    moves[gate.name] = f"Released {release_count}"

    # Step 2: Other nodes move aircraft forward
    for i in range(len(st.session_state.nodes) - 1, -1, -1):  # iterate backward
        node = st.session_state.nodes[i]
        if i == len(st.session_state.nodes) - 1:  # TRACON 2 -> Completed
            capacity = node.roll_capacity()
            moved = min(capacity, len(node.queue))
            for _ in range(moved):
                node.queue.popleft()
                st.session_state.completed += 1
            moves[node.name] = f"Moved {moved} to Completed"
        else:
...
st.write(f"### Completed Flights: {st.session_state.completed}")
